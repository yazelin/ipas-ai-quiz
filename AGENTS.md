...
開 PR 前讀 `.github/pull_request_template.md`,逐項自查;PR 描述照該範本填(human 走網頁會自動帶出,agent 要自己讀)。
...
**這條不是可有可無。**任務做完、測試過了,你會覺得收工了;但這次踩到而文件沒寫的坑,只要沒寫回 repo,下一個接手的 agent 會再踩一次。記在你自己的 memory 或對話裡沒有用,別人 clone 下來的 agent 讀不到。

回報完成之前,逐項問自己:

1. 這次有沒有哪一步第一次做錯、要改才過?有 → 寫進本檔對應段落(症狀長怎樣、對策是什麼)。
2. 那個錯會不會**靜默**發生(程式不報錯、只是結果不對)?會 → 加一條硬檢查進 `tools/check-questions.mjs` 或 `core.test.mjs`,並做負控制:故意注入那個錯,確認它真的擋得下來。
3. 那個步驟會不會重複做(例如每一梯次都要抽 PDF)?會 → 固化成 `tools/` 裡的腳本,本檔改成「用這支,不要自己硬幹」。

三項都是「沒有」就不用寫。寫了的話,在 PR 描述或回報裡列出補了哪幾條。

實例:9/18 加 115-3 梯次(#43)時踩到 PDF 頁尾混進題幹、`-layout` 行序錯亂,當下修好卻沒寫回;三天後才另開 #44 固化成 `extract-exam.mjs` 與 `check-questions.mjs` 第 5 項。這條規則就是要讓那一步在同一次做完。
