...
- **練習模式**：多選「官方章節」範圍 + 關鍵字過濾 + 三種出題（智慧複習/隨機）；答完即時看正解、詳解、對應教材連結，可寫筆記、加星標。
  - 智慧複習：優先出「答錯的 → 沒做過的 → 還沒掌握的」，讓你的時間花在還不會的題上。
- **模擬考模式**：整份試卷計時作答，交卷前不給答案，結束給分並可逐題檢討。
- **錯題本**：答錯會留著，同一題之後**連續答對 2 次**算掌握、自動移出；一鍵只練錯題。做法是紙卡背單字那一套：答對往前一格、答錯退回第一格。（**依答對次數升降格，沒有時間間隔**，不是排程式的間隔重複。）
- **學習統計**：涵蓋率、近期正確率（最近 50 題）、各範圍正確率/掌握度、成就徽章、最近 14 天題數趨勢。
- **每日**：今日挑戰（每天固定 3 題）、每日 AI 觀念卡（**會優先挑你還沒掌握的章節**）、每日目標 / 連續打卡 / 考前倒數。
  倒數**不必自己設定**：依你練過的題判斷是初級還是中級（題目 id 內含級別），自動對準官方公告的下一場；在設定裡填過日期就以你填的為準。
...
共 **890 題**，分兩個來源（練習頁可用「來源」下拉分開練）:

**歷屆公告試題 711 題**(`source` 缺省 = 歷屆)
- 初級：114 年第四次 / 115 年第一次 / 115 年第二次 / 115 年第三次 — 科目 1、2
...
- **題庫已對齊官方學習指引勘誤表**(初級 11404 版、中級 1150410 版):勘誤中的答案/題目修正已入庫,其餘勘誤項經全庫比對確認無殘留。
...
node core.test.mjs            # 核心邏輯測試(錯題升降格/計分/統計/出題優先序/匯出)
```
...
npx wrangler d1 create ipas-quiz-sync          # 把 database_id 填進 wrangler.toml
npx wrangler d1 execute ipas-quiz-sync --remote --file schema.sql   # 建表
