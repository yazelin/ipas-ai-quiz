...
4. **帶圖題**(題幹有「下圖/程式碼/圖表」純文字抽不到的):用 `pdfimages -png -p '<pdf>' /tmp/e` 取**乾淨內嵌圖**(忽略每頁固定尺寸的頁首裝飾圖;**不要用含左側答案欄的整頁截圖**)。圖**命名為 `<該題 id>.png`** 存到 `assets/`(例如 `assets/115-2-m-s3-f1.png`),該題填 `image: "assets/<id>.png"`。別沿用 PDF 原始圖號(早期 114-2 沒這樣、踩過坑);題幹寫「附圖」就一定要掛上。中文標點用全形(夾純英文括號維持半形)。
